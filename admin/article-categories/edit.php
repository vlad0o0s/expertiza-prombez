...
// Получаем список категорий для выбора родителя (исключая текущую и её потомков)
$allCategories = getAllArticleCategories();
$excludeIds = [$id]; // Исключаем текущую категорию
function getDescendantIds($categoryId, $allCategories)
{
    ...
    foreach ($allCategories as $cat) {
        if ($cat['parent_id'] == $categoryId) {
            $ids[] = $cat['id'];
            $ids = array_merge($ids, getDescendantIds($cat['id'], $allCategories));
        }
    }
    return $ids;
}
$excludeIds = array_merge($excludeIds, getDescendantIds($id, $allCategories));
$availableCategories = array_filter($allCategories, function ($cat) use ($excludeIds) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT id FROM article_categories WHERE slug = ? AND id != ?");
            $stmt->execute([$slug, $id]);
            if ($stmt->fetch()) {
                $error = 'Категория с таким URL уже существует';
            } else {
                try {
                    $stmt = $pdo->prepare("UPDATE article_categories SET name = ?, slug = ?, parent_id = ?, level = ?, description = ?, sort_order = ? WHERE id = ?");
                    $stmt->execute([$name, $slug, $parent_id, $level, $description ?: null, $sort_order, $id]);

                    header('Location: /admin/article-categories?success=1');
                    exit;
                } catch (PDOException $e) {
                    $error = 'Ошибка при сохранении: ' . $e->getMessage();
                    error_log("Article category update error: " . $e->getMessage());
                }
            }
        } catch (PDOException $e) {
    ...
    // Обновляем данные категории для формы
...
$pageTitle = 'Редактировать категорию - Админ-панель';
        ...
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <input type="number" id="level" name="level" min="1" max="3"
                    value="<?php echo htmlspecialchars($category['level']); ?>" required>
